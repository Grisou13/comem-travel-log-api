...
const PIPELINE = addRelatedPropertyPipelineFactory(Place, "Trip", [
  "apiId",
  "title",
]);
...
exports.createPlace = route(async (req, res) => {
  ...
  placesLogger.info(
    `Created place ${place.apiId} named "${place.name}" in trip ${place.trip.apiId}`
  );
...
exports.retrieveAllPlaces = route(async (req, res) => {
  const paginatedPipeline = await paginate(
    req,
    res,
    Place,
    PIPELINE,
...
exports.canModify = function (req) {
  return (
    ...
    req.place.trip &&
    req.place.trip.user &&
    req.currentUser.apiId === req.place.trip.user.apiId
  );
};
...
async function authorizeCreatePlace(req, place) {
  ...
  if (
    !req.currentUser ||
    !place._trip.user ||
    req.currentUser.apiId !== place._trip.user.apiId
  ) {
...
async function createPlaceFilters(req) {
        ...
        $or: toArray(req.query.bbox).map((bbox) => {
          ...
          if (parts.length !== 4) {
            throw invalidQueryParamError(
              "bbox",
              `Query parameter "bbox" must be a comma-delimited string of 4 numbers (longitude and latitude of south-west corner, and longitude and latitude of north-east corner), but it has ${parts.length} parts`
            );
          }
          ...
          if (notNumbers.length) {
            throw invalidQueryParamError(
              "bbox",
              `Query parameter "bbox" must be a comma-delimited string of 4 numbers (longitude and latitude of south-west corner, and longitude and latitude of north-east corner), but some of its values are not numbers: ${notNumbers
                .map((value) => `"${value}"`)
                .join(", ")}`
            );
          }
          ...
          if (coordinates[0] < -180 || coordinates[0] > 180) {
            throw invalidQueryParamError(
              "bbox",
              `Query parameter "bbox" must have a longitude between -180 and 180 as its first number, but the value is ${coordinates[0]}`
            );
          } else if (coordinates[1] < -90 || coordinates[1] > 90) {
            throw invalidQueryParamError(
              "bbox",
              `Query parameter "bbox" must have a latitude between -90 and 90 as its second number, but the value is ${coordinates[1]}`
            );
          } else if (coordinates[2] < -180 || coordinates[2] > 180) {
            throw invalidQueryParamError(
              "bbox",
              `Query parameter "bbox" must have a longitude between -180 and 180 as its third number, but the value is ${coordinates[2]}`
            );
          } else if (coordinates[3] < -90 || coordinates[3] > 90) {
            throw invalidQueryParamError(
              "bbox",
              `Query parameter "bbox" must have a latitude between -90 and 90 as its fourth number, but the value is ${coordinates[3]}`
            );
          } else if (coordinates[0] > coordinates[2]) {
            throw invalidQueryParamError(
              "bbox",
              `Query parameter "bbox" must have a south-west longitude smaller than its north-east longitude, but their values are ${coordinates[0]} > ${coordinates[2]}`
            );
          } else if (coordinates[1] > coordinates[3]) {
            throw invalidQueryParamError(
              "bbox",
              `Query parameter "bbox" must have a south-west latitude smaller than its north-east latitude, but their values are ${coordinates[1]} > ${coordinates[3]}`
            );
          }
      ...
      $match: {
        $or: toArray(req.query.near).map((bbox) => {
          ...
          if (parts.length < 3 || parts.length > 4) {
            throw invalidQueryParamError(
              "near",
              `Query parameter "near" must be a comma-delimited string of 3 or 4 numbers (longitude, latitude, optional altitude and distance), but it has ${parts.length} parts`
            );
          }
          ...
          if (notNumbers.length) {
            throw invalidQueryParamError(
              "near",
              `Query parameter "near" must be a comma-delimited string of 3 or 4 numbers (longitude, latitude, optional altitude and distance), but some of its values are not numbers: ${notNumbers
                .map((value) => `"${value}"`)
                .join(", ")}`
            );
          }
          ...
          if (numbers[0] < -180 || numbers[0] > 180) {
            throw invalidQueryParamError(
              "near",
              `Query parameter "near" must have a longitude between -180 and 180 as its first number, but the value is ${numbers[0]}`
            );
          } else if (numbers[1] < -90 || numbers[1] > 90) {
            throw invalidQueryParamError(
              "near",
              `Query parameter "near" must have a latitude between -90 and 90 as its second number, but the value is ${numbers[1]}`
            );
          }
          ...
          if (maxDistance <= 0) {
            throw invalidQueryParamError(
              "near",
              `Query parameter "near" must have a distance greater than zero as its last number, but the value is ${maxDistance}`
            );
          }
  ...
  if (req.query.href) {
    filters.push({
      $match: {
        apiId: {
          $in: toArray(req.query.href).map((value) =>
            value.replace(
              new RegExp(`^${escapeRegExp(`${Place.apiResource}/`)}`),
              ""
            )
          ),
        },
  ...
  if (req.query.search) {
    filters.push({
      $match: {
        $or: toArray(req.query.search).reduce(
          (memo, value) => [
            ...memo,
            {
              name: new RegExp(escapeRegExp(value), "i"),
